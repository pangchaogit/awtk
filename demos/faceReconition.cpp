...
int main()
{
    ...
    cascade.load("../model/haarcascade_frontalface_alt2.xml");//感觉用lbpcascade_frontalface效果没有它好，注意哈！要是正脸

    model = FisherFaceRecognizer::create();
    //1.加载训练好的分类器
    model->read("../model/MyFacePCAModel.xml");// opencv2用load
    //3.利用摄像头采集人脸并识别
    while (1)
    {
        cap >> frame;
        ...
        for (int i = 0; i < faces.size(); i++)
        {
            ...
            {
                case 2:str = "lijiaqi"; break;
                case 4:str = "pangchao"; break;
